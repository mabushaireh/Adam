...
namespace i2fam.DAL.Core
{
    ...
    public class AccountManager : IAccountManager
    {
        ...
        public async Task<IList<ApplicationUser>> GetUsersWithClaimAsync(string searchClaim)
        {
            var roles = await this._roleManager.Roles.Where(p => p.Claims.Any(claim => claim.ClaimValue == searchClaim)).ToListAsync();
            var rolesIds = roles.Select(role => role.Id);
            
            var users = await this._userManager.Users
                .Include(u => u.Roles).ToListAsync();
            var query = users.Where(user => CheckIfAnyOfRolesExist(user, rolesIds)).ToAsyncEnumerable();

            return await query.ToList();
        }
        private bool CheckIfAnyOfRolesExist(ApplicationUser user, IEnumerable<string> rolesIds)
        {
            var isRoleExist = false;
            foreach (var role in user.Roles)
            {
                if (rolesIds.Contains(role.RoleId))
                {
                    isRoleExist = true;
                    break;
                }
        ...
        public Task<bool> TestCanDeleteUser(string userId)
        {
