...
export class AccountService {
    ...
    updateUser(user: UserEdit) {
        if (user.id) {
            ...
            return this.accountEndpoint.getUserByUserNameEndpoint(user.email)
                .map((response: Response) => <User>response.json())
                .mergeMap(foundUser => {
    ...
    deleteUser(userOrUserId: string | UserEdit): Observable<User> {
        ...
        if (typeof userOrUserId === 'string' || userOrUserId instanceof String) {
            ...
            if (userOrUserId.id) {
                ...
                return this.accountEndpoint.getUserByUserNameEndpoint(userOrUserId.email)
                    .map((response: Response) => <User>response.json())
    ...
    changeUserStatus(userId: string, status: boolean) {
        if (status)
            return this.accountEndpoint.getActivateUserEndpoint(userId);
        else
            return this.accountEndpoint.getDeactivateUserEndpoint(userId);
    }
